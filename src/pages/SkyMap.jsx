...
export default function SkyMap() {
  ...
  const [viewDirection, setViewDirection] = useState("north");
  const [imageModalOpen, setImageModalOpen] = useState(false);
  const planisphereRef = useRef(null);

  const starChartImage = viewDirection === "north" ? NORTH_IMAGE : SOUTH_IMAGE;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-white mb-2">Hawaiian Sky Planisphere</h1>
        <p className="text-white/70 text-lg">Interactive star wheel showing Hawaiian constellations</p>
      </div>
            ...
            <CardContent className="p-6">
              ...
              >
                ...
                <div
                  ...
                  style={{ transform: `rotate(${rotationAngle}deg)`, transition: isDragging ? 'none' : 'transform 0.1s ease-out' }}
                >
                  <svg className="w-full h-full" viewBox="0 0 400 400">
                    <defs>
                      <mask id="viewingWindowNorth"><rect width="400" height="400" fill="white" /><path d="M 0 200 Q 200 280 400 200 L 400 0 L 0 0 Z" fill="black" /></mask>
                      <mask id="viewingWindowSouth"><rect width="400" height="400" fill="white" /><path d="M 0 220 Q 200 140 400 220 L 400 0 L 0 0 Z" fill="black" /></mask>
                      <clipPath id="circleClip"><circle cx="200" cy="200" r="200" /></clipPath>
                      <linearGradient id="blueGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                        <stop offset="0%" stopColor="#E0F2FE" /><stop offset="40%" stopColor="#BAE6FD" /><stop offset="100%" stopColor="#7DD3FC" />
                      </linearGradient>
                      <path id="curveTop" d="M 50 300 Q 200 355 350 300" fill="none" />
                      <path id="curveBottom" d="M 60 335 Q 200 370 340 335" fill="none" />
                    </defs>
                    <g clipPath="url(#circleClip)">
                      ...
                      <text fill="url(#blueGradient)" fontSize="18" fontWeight="bold" letterSpacing="2" stroke="#BAE6FD" strokeWidth="0.5">
                        <textPath href="#curveTop" startOffset="50%" textAnchor="middle">STARGAZERS ANONYMOUS</textPath>
                      </text>
                      <text fill="url(#blueGradient)" fontSize="28" fontWeight="bold" letterSpacing="4" stroke="#BAE6FD" strokeWidth="0.5">
                        <textPath href="#curveBottom" startOffset="50%" textAnchor="middle">MAUI</textPath>
                      </text>
                      ...
                      {months.map((month, i) => {
                        ...
                        return (<text key={month} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="12" fontWeight="bold" transform={`rotate(${i * 30}, ${x}, ${y})`}>{month}</text>);
                      })}
                      ...
                      {Array.from({ length: 24 }, (_, i) => {
                        ...
                        return i % 2 === 0 ? (<text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="10" opacity="0.8">{i}</text>) : null;
                      })}
                    </g>
                  </svg>
                </div>
                ...
                <div className="absolute inset-0 pointer-events-none">
                  <svg className="w-full h-full" viewBox="0 0 400 400">
                    <g><rect x="197" y="5" width="6" height="25" fill="#ef4444" rx="3" /><polygon points="200,30 195,35 205,35" fill="#ef4444" /></g>
                  </svg>
                </div>
              </div>

              <div className="text-center mt-4">
                <Button onClick={() => setRotationAngle(0)} className="bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 shadow-lg font-semibold">
                  <RotateCw className="w-4 h-4 mr-2" />Reset Position
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-6">
          <Card className="bg-gradient-to-br from-[#60A5FA]/20 to-[#3b82f6]/20 border-[#a855f7]/30 backdrop-blur-sm">
            <CardContent className="p-4">
              ...
              <div className="space-y-3">
                <div><p className="text-white/50 text-xs">Month</p><p className="text-white text-2xl font-bold">{months[monthIndex]}</p></div>
                <div className="pt-3 border-t border-white/10"><p className="text-white/50 text-xs mb-1">Rotation</p><p className="text-white">{Math.round(((rotationAngle % 360) + 360) % 360)}°</p></div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-[#a855f7]/20 to-[#ec4899]/20 border-[#a855f7]/30">
            <CardContent className="p-4">
              ...
              <div className="space-y-2 text-white/80 text-sm">
                <p>This digital planisphere shows the Hawaiian night sky for any month of the year.</p>
                <p>Rotate the outer wheel to align your desired month with the red indicator at the top.</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border-emerald-500/30">
            <CardContent className="p-4">
              ...
              <p className="text-white/90 text-sm">This planisphere features traditional Hawaiian star patterns used by ancient navigators.</p>
            </CardContent>
          </Card>
        </div>
      </div>

      <PlanisphereModal open={imageModalOpen} onOpenChange={setImageModalOpen} starChartImage={starChartImage} rotationAngle={rotationAngle} viewDirection={viewDirection} months={months} />
    </div>
  );
}
